...
import importlib
from ailawfirm_singapore.brain.intents import Intent, AGENT_FOR_INTENT


def route(intent: Intent, payload: str) -> dict:
    agent_module_name = AGENT_FOR_INTENT.get(intent, "matter_agent")
    full_module = f"ailawfirm_singapore.agents.{agent_module_name}"
    try:
...
def think(text: str) -> dict:
    ...
    try:
        from ailawfirm_singapore.brain import specialists

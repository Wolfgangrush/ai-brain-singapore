...
Talks to whatever AI host the app was launched under:
  ...
  * ``connect-local``            — writes ``ai_provider: "ollama"`` into
                                  ``~/.ailawfirm-singapore/config.json`` and the
...
# Jurisdiction root for the local-AI bridge config file. This is the same
# file ``connect_local.write_config()`` writes (and the one the user sees in
# onboarding), so reader + writer agree on the path. Centralising the
# path here means there is exactly one place to change it if a future
# edition relocates the config dir.
_LOCAL_CONFIG_FILE_ATTR = "CONFIG_FILE"

# Singapore's BrainConfig stores the audit log at <config_dir>/audit/.
# This is the singapore-equivalent of the india/usa default and matches
# the jurisdiction prefix used by ``connect_local`` (~/.ailawfirm-singapore).
_DEFAULT_AUDIT_CONFIG_DIR = os.path.expanduser("~/.ailawfirm-singapore")
...
def _read_provider_config():
    """Read provider selection from ``~/.ailawfirm-singapore/config.json``.

    Returns ``(provider, ollama_host, ollama_model)`` — all strings, all empty
    when unset / missing / unreadable. Reads via the same path that
    ``connect_local.write_config()`` writes, so a user who runs
    ``connect-local`` and then asks a question is honoured: there is no
    parallel config plumbing. A failure here is non-fatal: returning empty
    values simply means "no local preference" and the env-based cloud path
    takes over.
    ...
    """
    try:
        from ailawfirm_singapore import connect_local

        config_file = getattr(connect_local, _LOCAL_CONFIG_FILE_ATTR, None)
        if not config_file:
            return "", "", ""
        # `pathlib.Path.read_text` is a single syscall + decode; we wrap in
        # `os.fspath` so both str and Path are accepted.
        text = config_file.read_text()  # type: ignore[union-attr]
        cfg = json.loads(text)
    except Exception:
        return "", "", ""
    return (
        str(cfg.get("ai_provider") or "").strip().lower(),
        str(cfg.get("ollama_host") or "").strip(),
        str(cfg.get("ollama_model") or "").strip(),
    )
...
def available() -> bool:
    ...
    * a local provider is selected in ``~/.ailawfirm-singapore/config.json``
...
def _local_complete(system, user, max_tokens, timeout, ollama_host, ollama_model):
    ...
    from ailawfirm_singapore.pseudonymisation import PseudonymisationGateway
    from ailawfirm_singapore.pseudonymisation_audit import write_audit
    ...
    try:
        from ailawfirm_singapore.config import BrainConfig

        _cfg_dir = BrainConfig().config_dir
    except Exception:
        _cfg_dir = _DEFAULT_AUDIT_CONFIG_DIR
    try:

...
```bash
python3 -m ailawfirm_singapore reception   # greet + systems-check + memory + recap
python3 -m ailawfirm_singapore ask "<q>"   # route one query through the brain
python3 -m ailawfirm_singapore chat        # interactive REPL
python3 -m ailawfirm_singapore recap       # recent-session retrospective memory
```

...
def test_llm_complete_audits_and_still_replies(monkeypatch, tmp_path):
    from ailawfirm_singapore.brain import llm
...
def test_sg_nric_is_masked():
    """SG: NRIC must be masked."""
    gw = PseudonymisationGateway()
    clean, _ = gw.sanitize("The claimant is S1234567D per the records.")
    assert "S1234567D" not in clean

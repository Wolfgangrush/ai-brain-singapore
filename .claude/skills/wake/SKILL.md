---
name: wake
description: Start-of-session state loader for Singapore AI Brain users. Runs 2-pass leak-check (Rule 1: same scope as retrospective — no internal-shorthand, no internal-paths, no personal-build references). Loads jurisdiction context so every session begins informed.
allowed-tools: Bash, Read, Glob
---

# /wake — Start-of-Session State Loader (Singapore)

Loads the Singapore AI Brain's current state at session start. Displays readiness so the user knows exactly what's available before they begin work.
...
echo "Package: $(pip show ailawfirm_singapore 2>&1 | head -3)"
echo "Data dir: $(ls -la ~/.ailawfirm-singapore/ 2>&1 | head -5)"
echo "Config: $(cat ~/.ailawfirm-singapore/config.json 2>&1 | head -10)"
```

### Step 3: Jurisdiction context
Display the Singapore-specific legal framework:
- **Supreme Court of Singapore** (Court of Appeal + High Court) + **State Courts** + **SICC** (Singapore International Commercial Court)
- **Statutes:** Legal Profession Act 1966 (Cap 161), Evidence Act 1893, Penal Code 1871, Supreme Court of Judicature Act 1969, PDPA 2012, Companies Act 1967
- **Arbitration:** SIAC (Singapore International Arbitration Centre), IAA (Cap 143A)
- **Regulatory:** SAL (Singapore Academy of Law), MinLaw, Law Society of Singapore
...
```markdown
## 🇸🇬 Singapore AI Brain — Ready

🟢 Leak-check: PASS (Pass 1 clean · Pass 2 clean)

**System:**
- Python: [version]
- Package: ailawfirm_singapore [version]
- Data: ~/.ailawfirm-singapore/ [status]

**7 specialists online.**
**Supreme Court + State Courts + SICC mapped.**
**Legal Profession Act · PDPA 2012 · Evidence Act loaded.**

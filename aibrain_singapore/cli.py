...
aibrain_singapore ask "..."); this module only owns dispatch.

Front-desk commands (reception / ask / chat / recap):
    python3 -m aibrain_singapore reception
    python3 -m aibrain_singapore ask "validate [2023] SGCA 42"
    python3 -m aibrain_singapore chat
...
def cmd_reception(args):
    """Boot the brain — greet, verify all specialists, turn on retrospective memory, show recap."""
...
def main():
    ...
    parser.add_argument(
        "--palace",
        default=None,
        help="Where the palace lives (default: from ~/.aibrain-singapore/config.json or ~/.aibrain-singapore/palace)",
    )
    ...
    p_ask = sub.add_parser(
        "ask",
        help="Ask the brain one question — it classifies + routes to the right Singapore specialist",
    )
    p_ask.add_argument(
        "query",
        nargs="+",
        help='Your question, e.g. "limitation for a contract claim filed 12 Jan 2026"',
    )

    # chat — interactive terminal session with the brain
    sub.add_parser(
        "chat",
        help="Talk to the brain in the terminal — each line routes to a Singapore specialist",
    )

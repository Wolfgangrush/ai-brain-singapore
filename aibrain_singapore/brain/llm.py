...
Talks to whatever AI host the app was launched under:
  ...
  * ``connect-local``            -- writes ``ai_provider: "ollama"`` into
                                  ``~/.aibrain-singapore/config.json`` and the
...
The provider is selected by ``ai_provider`` in ``~/.aibrain-singapore/config.json``.
...
def _read_provider_config():
    """Read provider selection from ``~/.aibrain-singapore/config.json``.
    ...
    """
    try:
        from aibrain_singapore.config import BrainConfig
...
def available():
    ...
    * a local provider is selected in ``~/.aibrain-singapore/config.json``
...
def _local_complete(system, user, max_tokens, timeout, ollama_host, ollama_model):
    ...
    """
    import sys as _sys
    from aibrain_singapore.pseudonymisation import PseudonymisationGateway
    from aibrain_singapore.pseudonymisation_audit import write_audit
    ...
    try:
        ...
        _cfg_dir = os.path.expanduser("~/.aibrain-singapore")
    try:

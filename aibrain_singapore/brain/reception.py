...
from aibrain_singapore.brain import memory
...
def run_reception() -> int:
    ...
    for name in AGENTS:
        try:
            mod = importlib.import_module(f"aibrain_singapore.agents.{name}")
    ...
    router_ok = False
    try:
        router = importlib.import_module("aibrain_singapore.brain.router")

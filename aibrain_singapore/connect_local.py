...
Usage from CLI:
    ...
    aibrain-singapore connect-local --yes      # skip prompts
    aibrain-singapore connect-local --model qwen3:7b   # force a specific model

Result: ~/.aibrain-singapore/config.json is written with ai_provider=ollama and the
...
def install_ollama_instructions() -> None:
    ...
    print()
    print("Then run `aibrain-singapore connect-local` again.")
...
def cmd_connect_local(args) -> int:
    ...
    print("  aibrain-singapore connect-cloud --provider deepseek --cloud-warning-acknowledged")
    print()
    return 0
